...
// Defines a position and similarity score [0, 1] for a match
export type Match = {
    position: {
        x: number;
        y: number;
...
// handle much larger than this in a reasonable time frame
export const MAX_SEARCH_DOMAIN = 250 * 250 * 3;
export const MAX_TEMPLATE_DOMAIN = 50 * 50 * 3;
        ...
        for (let s_x = 0; s_x <= searchImage.width - templateImage.width; s_x += searchAdvanceX) {
            ...
            for (let t_y = 0; t_y < templateImage.height; t_y++) {
                for (let t_x = 0; t_x < templateImage.width; t_x++) {
                    ...
                    const channels = Array.from({ length: searchImage.channels }).map((_, index) => index);
